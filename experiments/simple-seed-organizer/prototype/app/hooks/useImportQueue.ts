...
export type QueueStatus = 'pending' | 'extracting' | 'ready' | 'saving' | 'saved' | 'needs_review';
...
function toSeedPayload(seedId: string, extracted: AIExtractedData, photoFrontPath: string): Omit<Seed, 'createdAt' | 'updatedAt'> {
  ...
  return {
    ...
    notes: [extracted.description, extracted.plantingInstructions]
      .filter(Boolean)
      .join('\n\n') || undefined,
    photoFrontPath,
  };
...
export function useImportQueue({ userId }: UseImportQueueOptions) {
  ...
  const saveQueueItem = useCallback(async (item: QueueItem) => {
    ...
    try {
      ...
      const saved = await saveSeed(toSeedPayload(seedId, item.extracted, photoFrontPath));
      updateItem(item.id, { status: 'saved', savedSeed: saved, errorMessage: undefined });
  ...
  const extractQueueItem = useCallback(async (item: QueueItem) => {
    activeRef.current += 1;
      ...
      if (!res.ok) {
        updateItem(item.id, {
          ...
          errorMessage: res.status === 402
            ...
            : (json.message || "I couldn't read that packet. Try again or enter details manually."),
        });
        autoSaveIdsRef.current.delete(item.id);
        return;
      }

      updateItem(item.id, { status: 'ready', extracted: json.data, errorMessage: undefined });
    } catch (err) {
      updateItem(item.id, {
        status: 'needs_review',
        errorMessage: err instanceof Error
  ...
  useEffect(() => {
    ...
    const availableSlots = Math.min(CONCURRENCY - activeRef.current, pending.length);
  ...
  const enqueuePileItems = useCallback((itemsToEnqueue: QueuePileItem[], options?: { autoSaveOnReady?: boolean }) => {
  ...
  const enqueueFiles = useCallback((files: FileList | File[], options: EnqueueOptions) => {
    const acceptedFiles = Array.from(files).filter((file) => file.type.startsWith('image/'));
    if (acceptedFiles.length === 0) return;
    enqueueCapturedImages(acceptedFiles.map((file) => ({ file })), options);
  }, [enqueueCapturedImages]);

  const retryItem = useCallback((itemId: string, options?: { autoSaveOnReady?: boolean }) => {
    if (options?.autoSaveOnReady) {
  ...
  const updateAfterManualSave = useCallback((itemId: string, savedSeed: Seed) => {
    autoSaveIdsRef.current.delete(itemId);
    updateItem(itemId, { status: 'saved', savedSeed, errorMessage: undefined });
  }, [updateItem]);

  const pendingCount = items.filter((item) => item.status === 'pending').length;
  const processingCount = items.filter((item) => item.status === 'extracting' || item.status === 'saving').length;
