...
export async function POST(request: NextRequest) {
  try {
    ...
    if (seed.user_id && seed.user_id !== user.id) {
    ...
    if (!canUseAICount(aiCompletions, tier, 1)) {
      return NextResponse.json(
        { error: 'AI limit reached', message: 'Upgrade to use more AI features this month.' },
        { status: 402 }
      );
    }
...
- Only fill in fields that are standard for this variety; use null otherwise`;
    ...
    const sunReq = (v: unknown): 'full-sun' | 'partial-shade' | 'full-shade' | undefined => {
      if (v === 'full-sun' || v === 'partial-shade' || v === 'full-shade') return v;
      return undefined;
    };
    ...
    if (enrichedFields.length > 0) {
      const result = await updateSeed(seedId, updates);
      if (result) updatedSeed = result;
    }
    ...
    return NextResponse.json({ success: true, seed: updatedSeed, enriched: enrichedFields });
  } catch (error) {
