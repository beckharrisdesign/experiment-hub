...
import { PLANS, getTierIndex, normalizeTier } from "@/lib/plans";
...
export function ProfileTierTable({
  ...
  const currentIdx = getTierIndex(normalized);
            ...
            <ul className="space-y-1 text-sm text-gray-700 mb-4 flex-1">
              <li className="flex gap-2">
                ...
                {plan.seeds === "Unlimited"
                  ...
                  : `${plan.seeds} seed packets`}
              </li>
              <li className="flex gap-2">
                <span className="text-[#16a34a] shrink-0">✓</span>
                {plan.ai === "Unlimited"
                  ? "Unlimited AI packets"
                  : `${plan.ai.replace("/month", "")} AI packets/month`}
              </li>
            </ul>
