    ...
    >
      ...
      <div className="w-12 h-12 rounded-lg shrink-0 overflow-hidden bg-gray-100">
        {seed.photoFront ? (
          ...
          <div
            className={`w-full h-full ${ageColor} flex items-center justify-center`}
          >
            <svg
      ...
      {seed.year &&
        (getViabilityStatus(seed.year, undefined, seed.name) !== "good" &&
        getViabilityStatus(seed.year, undefined, seed.name) !== "unknown" ? (
          <ViabilityBadge year={seed.year} cropName={seed.name} />
        ) : (
