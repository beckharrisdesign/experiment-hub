...
interface SeedListProps {
  ...
  /** Total seeds in inventory before filtering — used to distinguish empty search from empty inventory */
...
function groupByAge(seeds: Seed[]): Record<string, Seed[]> {
  ...
  seeds.forEach((seed) => {
    ...
    } else if (isUseFirst(seed.year, undefined, seed.name)) {
...
export function SeedList({
      ...
      return (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-[#99a1af]">
            Add photos when creating or editing seeds to see them here
          </p>
        </div>
      );
    }
  ...
  if (seeds.length === 0) {
    const isFilteredEmpty = totalSeedCount !== undefined && totalSeedCount > 0;
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center mb-4">
          <svg
        ...
        {isFilteredEmpty ? (
          <>
            <p className="text-[#6a7282] mb-2">No seeds match your search</p>
            <p className="text-sm text-[#99a1af]">
              Try a different name, variety, or filter
            </p>
          </>
        ) : (
          <>
        ...
        <div key={groupName}>
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-[#4a5565] font-semibold">
              {viewMode === "type"
                ? TYPE_LABELS[groupName] || groupName
                : groupName}
            </h2>
