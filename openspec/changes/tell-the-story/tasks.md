...
- [x] 2.2 **Done 2026-07-22 (Katy)** — `NOTION_HISTORY_DATA_SOURCE_ID` added to env (local + Vercel) and the database shared with the hub's integration. Verified live: test entries render on the production detail page.
...
- [x] 3.3a **Receipt link un-deferred 2026-07-22 (Katy's request):** `Receipt URL`, originally stored-only in v1, now renders as a small mono "receipt ↗" link after the milestone sentence. Adapter carries `receiptUrl` through `HistoryEntry` (null when unset → no link). BDE's three commit-backed entries got their commit URLs; the two account-only entries (signups, the stall) stay unlinked.
- [x] 3.3 `app/experiments/[slug]/page.tsx`: History band below the statements. 88px mono/tabular gutter (right-aligned, stacks over the sentence at `<sm`), 13px muted dates, `HISTORY` uppercase label. Empty → renders nothing. **Visual sign-off deferred to 4.3** — needs live entries + the env var, not observable locally. (→ 1.1, 1.2)
  - **Gutter and type specifics superseded 2026-08-23 (PR pending, this branch).** The band moved onto the MVDS ramp and spacing scale after a Figma As-is/Proposed pass (file `HKy2SdRDyCJ37V29mvMpma`, pages `03 As is` / `04 Proposed`; Katy approved the proposed frame in-session). Changes: gutter 88px → 128px and `whitespace-nowrap` (sized for `formatDateSpan`'s longest output, the 17-char cross-year span, at the mono 0.6em advance — a wrapped date read as two dates); dates 13px → 12px; milestones `text-sm leading-[1.7]` → `text-body`; source lines `text-[13px] leading-[1.6]` → `text-small`; `IMPACT SCORE` label → `text-caption`; tabs → `text-small`; entry gap 16 → 24px, source rail indent 12 → 16px, source line gap 6 → 8px (all MVDS `space-*` steps). Verified: 753/753 tests, utilities compiled from `app/globals.css` assert 16/1.6, 14/1.5, 12/1.4+0.01em.
...
- [ ] 3.11 **Date ranges, not months (decided by Katy 2026-08-22 while authoring the exemplar — month grain was the generator's convenience leaking into the narrative).** Notion `Date` gains optional end dates (no schema change; the property already supports them). Adapter carries `endDate` through `HistoryEntry`; `formatMonthYear` becomes a span-aware formatter: single day → "Mar 9, 2026", within-month span → "Mar 10–30, 2026", cross-month → "Apr–Jun 2026", month-wide (no end or full month) → "Apr 2026". Sort stays on start date. Generator-written monthly entries need no change (month-wide renders as before). Working ranges for the exemplar are in `exemplar-draft.md` §Staging notes.
...
- [ ] 4.3 (partially verified 2026-07-22 — test entries render on the production detail page; empty-state and approval-toggle checks still open) Manual walkthrough on the running app: the exemplar experiment shows History below the statements with dates aligned in the gutter; an experiment with no entries shows no band and no heading; unchecking `Approved` in Notion removes that entry within the 60s cache window. (→ 1.1, 1.2, 1.3)
