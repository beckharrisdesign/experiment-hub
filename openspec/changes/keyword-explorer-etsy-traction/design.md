...
| Libraries / version | `MVDS Core` (`lk-d54f86bc…`), subscribed to the file 2026-09-18 · `@beckharrisdesign/mvds@0.3.0` in code. Round 02.1 is rough (shape/composition) — not yet token-audited; the implementation binds real MVDS tokens (`Input`/`Field` for the range control, existing `Select`/`Badge` usage unchanged) rather than the hand-set colors in the rough frame. |
