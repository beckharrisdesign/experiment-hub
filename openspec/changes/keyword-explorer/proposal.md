...
**Colour coding per column value — recorded, not applied.** Katy: *"an interesting idea — I might change how it appears. but the concept is one I'd keep around."* So the schema is written down here and **deliberately left out of the first implementation**; `keyword-explorer`'s table rules forbid applying it until she asks. What is recorded: each numeric column bands its own value against MVDS semantic tokens — KD at ≤20 / 21–50 / >50 on `success` · `neutral` · `destructive`, and the same treatment for searches (volume), competition (crowding) and tag occurrences (saturation). Tokens rather than decoration, so a colour means something. The thresholds themselves are `design.md`'s problem.
