...
- [x] 1.3 User sees a watermarked preview of all 6 curated images with price + what they get *(Preview the pack)* — verified live 2026-07-27; layout/crop feedback open in #335/#330 (partly addressed by #337 restyle, re-verify). Preview-reveal feedback (scroll/focus/announce, #336) added by change `elk-preview-feedback` 2026-07-30 — include in the next production walkthrough (5.4)
...
- [x] 1.6 A signature-valid `checkout.session.completed` marks the order paid, builds the un-watermarked zip, exposes a signed download, sends one email *(Webhook fulfils once)* — live receipt 2026-07-27: paid 16:47:28Z → fulfilled 16:47:34Z, event id + output stored (elk_orders row `0484a635`), but `email_message_id` null (`RESEND_API_KEY` landed in Vercel after that deploy). **CLOSED 2026-08-08 on live data:** every order from 2026-08-03 onward carries a real `email_message_id` with `email_error` null (`3eeb0ffb`, `18b148bd`, `3f241132`, `83e5087d`) — the email half is verified in production, all 6 lifetime orders fulfilled, 0 refunds
...
- [~] 3.8 Analytics layer: typed funnel events wired (`landing_view`, `upload_started`, `preview_viewed`, `checkout_started`, `result_delivered`, `payment_cancelled`); GA4 client (gtag) + server purchase (Measurement Protocol, once on fulfillment); UTM/click-id persisted through Checkout — `lib/etsy-listing-kit/analytics.ts` (no-op without GA keys). **Superseded 2026-08-07:** the GA4 funnel half was real, but the Google Ads half never worked — see 3.11
- [x] 3.11 Ads conversion repair + funnel gap-fill (2026-08-07, verified in-browser via dataLayer): (a) `Sign-up` conversion pointed at the hub Ads account `AW-10904266222` instead of the campaign's `AW-277034089`, which was never `gtag('config')`'d — the test asserted the wrong id so it passed green; (b) `FormSubmit` (Ads action 6657647682, GA4 import) had **no backing event at all** and had never received a hit; (c) CSP blocked *every* Ads beacon (`googleadservices.com`, `google.com/ccm`, `*.doubleclick.net`) so even the pre-existing hub conversion was dead — invisible to unit tests, only surfaced in the browser console; (d) added start/terminal events (`upload_picker_opened`, `upload_rejected`, `preview_requested`, `preview_failed`, `checkout_failed`, `download_clicked`) + per-step durations, since every step previously had a success event but no start event, making failure and mid-step abandonment invisible; (e) ELK tagged as its own analytics surface + correct page metadata (it reported as generic "BHD Labs" hub traffic). Measured: preview generation ~6.7s, the longest in-page wait. **Still owner-only:** link GA4 `G-120M120GDY` → `AW-277034089` and confirm the Sign-up action's value setting, both in the Ads/GA4 UI; conversions cannot be confirmed live until deploy (CSP is a response header)
...
- [~] 6.3 Prepare (do not fund) the $1/day ad campaign + UTM convention; Katy performs turn-on — plan + UTM convention merged (PR #334). **First burst enabled 2026-08-04** (campaign `etsy-listing-kit-test1`, id 24091208777, $5/day × 3d). **That run is VOID** — Google Ads errors throughout, and Ads conversion tracking was dead for the whole window (repaired 2026-08-07 by #363/#364). Funnel truth for it: **0 ad-attributed orders** — no `elk_orders` row from that window carries `utm_source`/`click_id`; all 6 lifetime paid orders are owner self-purchases. **Burst reset to day 1 = 2026-08-08** (`docs/AD_CAMPAIGN_GOOGLE.md`). Still owner-only before re-enable: link GA4 `G-120M120GDY` → `AW-277034089`, confirm the Sign-up action's value, bump `ELK_LAUNCHED_AT` to the re-enable timestamp + redeploy
