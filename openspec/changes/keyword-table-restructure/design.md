...
| Round 02.10 | Page `02.10 Proposed — one listing column` (`18:2`), frame `18:3`. The sub-row *is* the listing; every other listing fact is an attribute on that row. Drawn on raw tokens. |
| Round 02.11 *(current)* | Page `02.11 Proposed — MVDS` (`20:26`), frame `20:27`. **The MVDS round.** Same structure as 02.10, built on MVDS Core. |
...
| Libraries / version | **MVDS Core — subscribed 2026-09-21 by Katy ("mvds added"). Gate closed.** `get_libraries` confirms `MVDS Core` in `libraries_added_to_file`. Round 02.11 uses real `Select` instances from the library (`componentKey 9743e013…`). See Decision 17 for why the frame is **not** bound to MVDS's `Tokens` collection. |
...
| Status | Round 02.11 current, verified by screenshot. 02.3–02.10 kept; 02.7 remains the reference for scroll behaviour and 02.8 for the full column set. **MVDS gate closed.** |
...
**17 — The MVDS round is built on MVDS *components*, and deliberately not on MVDS's *tokens*.** Katy enabled the library on 2026-09-21 (*"mvds added"*), closing a gate carried since `keyword-captured-demand` task 4.4. `get_libraries` now returns `MVDS Core` in `libraries_added_to_file`, and round 02.11 places real `Select` instances imported from it.

**The first attempt at this round was wrong, and the repo had already written down why.** Binding the frame's fills to MVDS's `Tokens` collection and selecting its `Dark` mode produced a near-black greyscale table — because MVDS Core's own dark palette *is* greyscale. `app/globals.css` § *Brand layer* explains exactly this: the hub redefines MVDS's shadcn-convention tokens after importing its stylesheet, so *"every MVDS component rendered in MVDS's own greyscale: `<Button>` came out near-white … inside a dark green page, and **the pdf-metadata-viewer Figma frames — which show green buttons — did not match what the app actually shipped**."*

That is the same failure, in the same tool, one route over. **So "build the proposed frame on MVDS Core" cannot mean binding colour to MVDS's library variables.** In Figma a library variable's value cannot be overridden locally, so there is no way to express the brand layer as a binding. Round 02.11 therefore uses MVDS components and paints them in the hub's brand values — which is what the running app does to those same components at runtime.

**The route matters.** `globals.css` reserves `.efa-theme` and `.mvds-theme` for routes *meant* to look like plain MVDS (the assessment suite, `/svg-to-stitch`). `/keyword-explorer` is not one of them, and `KeywordTable` reads the hub's namespaced tokens throughout — `bg-background-primary`, `text-text-primary`, `border-accent-primary` — never MVDS's semantic names. A greyscale frame would have misrepresented the surface.
