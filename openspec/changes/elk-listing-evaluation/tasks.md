...
- [ ] 3.4 Flow states (PARTIAL: checking spinner, shop-link confirm card, unusable-URL error shipped; REMAINING: the flow per approved Figma 02.27 — `/check` as its own shareable route, checkout seeded from the scrape, `/result` gaining the GENERATING state; URL map panel on 02.27 is the routing reference. Supersedes the old "fix flow re-cut" framing: the evaluation exits into purchase, not upload)
- [ ] 3.5 Kit scope per approved Figma 02.26/02.27: ten new-or-updated images + reusable template from the generator, built via the wording pipeline (ingest → sourced listing brief → tee-up; scene copy quotes the brief only — ladder rules on the 02.26 scene-system frame); title/tags/alt-text suggestion generation. INTERIM (founder-approved honesty patch, 2026-08-30): production copy promises six images with everything else COMING SOON until this lands
