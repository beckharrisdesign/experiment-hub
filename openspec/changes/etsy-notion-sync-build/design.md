...
- **Tailwind major-version mismatch:** MVDS is built on Tailwind v4; the hub is on Tailwind v3.4. **Spike result (2026-07-15):** `@beckharrisdesign/mvds@0.2.0` (now on public npm, tokenless) ships Tailwind v4 *source* (`styles.css` opens with `@import "tailwindcss"`) and peer-depends on `tailwindcss: ^4` — there is no compiled-CSS option in the package, so consuming MVDS requires a Tailwind v4 build on the consumer side. **Founder decision (2026-07-15): upgrade the hub to Tailwind v4 as its own change** (`openspec/changes/hub-tailwind-v4/`, aligns with issue #285) — chosen over MVDS shipping a precompiled CSS artifact or a page-scoped v4 side-build. The sync page builds on MVDS natively once that change lands; it does not ship on a broken hybrid.
- ~~Private registry in CI/Vercel~~ **Resolved 2026-07-15:** MVDS is on the public npm registry (tokenless installs); no PAT is needed anywhere. The `.npmrc` `@beckharrisdesign:registry` line is deleted as part of `hub-tailwind-v4`.
