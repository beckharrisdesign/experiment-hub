...
- **Tailwind major-version mismatch:** MVDS is built on Tailwind v4 (`@import "@beckharrisdesign/mvds/styles.css"` + `@source` scanning of its dist); the hub is on Tailwind v3.4. First implementation task is a **spike**: verify MVDS components render correctly inside the hub (options: consume MVDS's compiled CSS alongside v3, scope a v4 build to the sync page, or upgrade the hub to v4 as a separate platform change). The spike's outcome decides; the page does not ship on a broken hybrid.
