...
- **Type badge bug surfaced during reconstruction:** `ExperimentTypeBadge` returned `null` for `commercial` — so Business-type experiments (Notion "Business" → hub `commercial`) rendered no type badge live. The proposed hero leans on badge + chip pairing. **Resolved (option a):** the `type === "commercial"` null guard was removed so a "Commercial" badge now renders like the other types (`typeStyles`/`typeLabels` already covered it); component test added. Behavior change to note — Business-type experiments now show a type badge in the hero and the homepage list.
