...
- Targeting reads current listing tags, refreshed at most every 60 seconds (a per-server-instance cache guards the live Supabase read against a crawler or refresh burst — Decisions, below); a Supabase failure degrades that one column, not the page.
...
**Targeting is computed server-side, not baked into the static corpus.** `getLatestListingSnapshots()` reads current listing `tags` from Supabase — "current" is the entire point of Targeting (a keyword that gets un-tagged tomorrow must stop showing a slot number tomorrow, not at the next `ingest-pulls.py --apply`). The Keyword Explorer route gains its first non-static data dependency. Implementation: `withTargeting()` (`lib/keyword-traction.ts`) is the I/O wrapper — it calls `getLatestListingSnapshots()`, computes a keyword→slot map via the pure `computeTargeting()`, and merges it onto the statically-loaded corpus rows; the page's server component just awaits `withTargeting()` and renders the result. `getLatestListingSnapshots()` itself holds a 60-second per-server-instance cache (added after review — Risks, below) so this is "current within the last minute," not strictly request-by-request; the corpus JSON and its static import are otherwise untouched.

**Supabase failure degrades Targeting only, silently.** If `getLatestListingSnapshots()` throws (missing env vars in a preview deploy, a transient Supabase error), `withTargeting()` (`lib/keyword-traction.ts`) — the I/O wrapper the page awaits, not the page's own server component — catches it, treats Targeting as unavailable for every row (blank, same as "no match"), and returns rows the page renders normally. No banner, no partial-page error state — consistent with Katy's round-01 feedback on `keyword-explorer` cutting the caveat banner ("the rule stays, I just don't need all the verbose slop"). The failure is logged server-side for anyone debugging it, not surfaced to the table.
...
**The per-listing detail never reaches the browser — added after review, not in the original round.** `KeywordTable` is a client component on a public, unauthenticated route (`/keyword-explorer` has no auth gate — confirmed against `middleware.ts`), so any prop it receives serializes into the RSC payload for anonymous visitors. The original implementation passed full `KeywordRow` objects straight through, including `ranked.matches` (ranked listings' titles, pages, positions) and `targeting.matches` (live listing IDs and which tag slot they occupy) — none of which the table renders; only `.best` is ever displayed. A `KeywordTableRow` type (`types/index.ts`) now sits at the server/client boundary: `ranked`/`targeting` are plain `number | null`, collapsed by a new `toTableRows()` (`lib/keyword-traction.ts`) that the page calls right before rendering. The full detail stays server-side on `KeywordRow`, available if a future change ever builds the per-row surface this one deliberately doesn't (Non-Goals) — it just never leaves the server today.

**Targeting's "no longer live" filter has two layers, not one.** Round-1 review flagged that a deleted/deactivated listing could read as permanently "targeted" and the fix (`state === "active"`) was accepted; round-2 review correctly pushed further: `getLatestListingSnapshots()`'s source view is newest-row-per-listing across *all history*, so a listing Etsy stops returning at all keeps its last snapshot (and whatever `state` it had then) forever — `state === "active"` alone can't see that. Fixed at the root: `getLatestListingSnapshots()` (`lib/etsy-sync.ts`) now also filters to rows stamped with the batch's newest `captured_at`, mirroring `experiments/etsy-notion-sync/prototype/store_supabase.py`'s `latest_from_current_capture` — the same rule already proven correct on the Python side of this same pipeline. `state === "active"` in `computeTargeting` remains as the second, narrower layer: a listing present in the latest capture but marked draft/inactive/expired still shouldn't count as "targeted."
...
**The Targeting read is cached for up to 60 seconds, added after review, not in the original round.** `/keyword-explorer` is `force-dynamic`, so absent a cache, every anonymous page load would trigger its own service-role Supabase read of `etsy_latest_listing_snapshots` — a crawler or a burst of refreshes could multiply that load with nothing to bound it, the same shape of risk `etsy-listing-kit`'s evaluate route already guards against with a cache/throttle. `getLatestListingSnapshots()` (`lib/etsy-sync.ts`) now holds a 60-second, per-server-instance TTL cache with concurrent callers coalesced onto one in-flight read. This is a real, if small, step back from "Targeting reflects the tags on Etsy right now": for up to a minute after a tag change, a page load can still show the previous value. Accepted because listing tags don't change on a sub-minute cadence in practice, and this is still far fresher than the corpus's own `ingest-pulls.py --apply` cadence; a failed read is never cached, so a Supabase outage doesn't get "stuck" past its own recovery.
