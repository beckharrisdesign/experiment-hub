...
- **Done when:**
  ...
  4. **The Exit** section names ship / kill / park / graduate, states that exit is available at any point (not a final phase), states the ledger rule — *lessons become diffs, not journal entries; the story is derived from commits, PRs, and artifacts, never written alongside them* — and shows at least two named real experiments with a one-line "what it taught" each (hand-written in v1). Parks carry a wake condition; kills carry a reason.
  5. **One real week** closes the page: a short dated ledger excerpt (the 2026-07-17 → 2026-07-21 `tell-the-story` slice) in "Katy said → we made → receipt" form, every receipt a real link (PR, commit, Figma file).
