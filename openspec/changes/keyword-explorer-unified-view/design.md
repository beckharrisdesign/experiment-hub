...
**5 — A repeat capture collapses to the newest, and the table stays silent about it.** Settled 2026-09-20, during apply, because the row shape could not be written without it. 83 keywords carry two Keyword Tool captures (2026-09-17 and 2026-09-18). The merged row shows the **newest** capture's Searches / Competition / KD; the earlier captures stay on the row's Keyword Tool sub-object as a `history` array, so nothing archived is discarded and `current` / `supersededBy` keep exactly the meaning they have today. The table does **not** mark that a row is a collapse. Katy chose this over a per-keyword capture-count affordance: that would be a new visual element, and it would need its own numbered Figma round before it could ship. If the silence later proves wrong, adding the marker is additive and does not change the corpus.

**6 — Mobile keeps every column.** 480px shows roughly three columns before scrolling, which is poor — but the alternatives are worse: hiding columns makes the sparseness invisible, and a card-per-keyword layout destroys the side-by-side comparison that is the entire point of merging. Horizontal scroll is the existing convention (`keyword-explorer-etsy-traction` § design) and it is kept.
...
**~~`current` / `supersededBy` is still unresolved~~ — settled during apply, see decision 5.** 2022 rows collapsing to 1939 keywords means 83 keywords carry more than one Keyword Tool capture. Implementation could not proceed without an answer, so it came back here as `tasks.md` § 3.17 required: newest capture wins, earlier captures retained as `history`, table silent. The residual risk is that a reader cannot tell a collapsed row from a single-observation row — accepted deliberately, and reversible by adding the marker later.
