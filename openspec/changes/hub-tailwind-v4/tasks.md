...
- [ ] 1.2 MVDS components render on the dev route — Button, Badge, Card, Section styled by MVDS tokens in the hub's dark context (hub pins `.dark` on `<html>`; light unreachable)
...
- [x] 3.4 Dev-only proof route `app/dev/mvds/page.tsx` (404s in production; visible in dev and Vercel previews) rendering Button/Badge/Card/Section in the hub's dark context

## 4. QA

- [x] 4.1 Full vitest suite + `npm run build` green on v4
- [ ] 4.2 Visual parity pass against production (Vercel preview): home, experiment detail, header/nav, dark sections — explicitly checking the v4 preflight traps (border color, rings, shadows); screenshot the MVDS proof route (dark — the hub's only mode)
