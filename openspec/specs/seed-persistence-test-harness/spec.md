...
Canonical capability for **Simple Seed Organizer** seed persistence: `saveSeed`, `updateSeed`, and `convertSeedToDbSeed` (plus photo path behavior) are covered by Vitest with mocked Supabase, JSONB empty-array rules, bounded async tests, and documented run commands. Promoted from archived change `seed-save-flow-test-coverage` (2026-05-19).
