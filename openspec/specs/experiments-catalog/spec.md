...
- **THEN** the system SHALL return records from `data/experiments.json` matching the persisted schema (identifiers, directories, linkage to docs and prototypes, status, scoring)
...
- **THEN** the system SHALL return the experiment whose slugified name matches or null when none match
