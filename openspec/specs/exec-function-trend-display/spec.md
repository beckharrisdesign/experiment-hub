# exec-function-trend-display

## Purpose

Every session recorded for a track is shown — nothing is filtered by score, because a dashboard that decides which of your sessions counted is doing the one thing a measurement tool must not. Points position by the instant they were recorded so two runs in one day are two marks, the chart spans its container at a fixed height, and each row carries the time, duration and trial count needed to judge a run without opening the database.
