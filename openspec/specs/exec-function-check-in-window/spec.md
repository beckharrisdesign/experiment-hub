# exec-function-check-in-window

## Purpose

The everyday check-in states the period it asks about — the past week — above the items and again in every subscale header, and records that window on each stored session. Long retrospective windows are reconstructed from salient incidents rather than averaged from experience, and here that reconstruction is performed by the executive function being measured, so a shorter stated window is the more rigorous choice. Sessions stored before any window was declared remain identifiable as such.
