...
const CONNECTOR_COLOR: RGB = { r: 0.44, g: 0.51, b: 0.6 }; // #6e7681
const LABEL_COLOR: RGB = { r: 0.79, g: 0.82, b: 0.85 };    // #c9d1d9
const BG_COLOR: RGBA = { r: 0.05, g: 0.07, b: 0.09, a: 1 }; // ~#0d1117
const CARD_FILL: RGBA = { r: 0.086, g: 0.106, b: 0.133, a: 1 }; // #161b22
const CARD_STROKE: RGB = { r: 0.188, g: 0.212, b: 0.239 };  // #30363d
const GROUP_LABEL_COLOR: RGB = { r: 0.345, g: 0.404, b: 0.463 }; // #58697680
...
function sendProgress(current: number, total: number, label: string) {
 ...
 */
function computeLayout(pages: ManifestPage[]): Map<string, { x: number; y: number }> {
  const positions = new Map<string, { x: number; y: number }>();

  // Build adjacency: path → children ids
  const childrenByPath = new Map<string | null, ManifestPage[]>();
  ...
  function layOutLevel(nodes: ManifestPage[], depth: number): number {
...
figma.ui.onmessage = async (msg: PluginMessage) => {
  if (msg.type !== "BUILD_SITEMAP") return;
  ...
  try {
    ...
    for (const pos of positions.values()) {
      ...
      if (pos.y + CARD_H + LABEL_H > maxY) maxY = pos.y + CARD_H + LABEL_H;
    }
    ...
    const cardNodes = new Map<string, { node: FrameNode; pos: { x: number; y: number } }>();
    ...
    for (let i = 0; i < pages.length; i++) {
      ...
      const card = await createPageCard(page, imageBytes);
      card.x = pos.x;
      card.y = pos.y;
    ...
    for (const page of pages) {
      ...
      createConnector(
        { x: parentCard.pos.x, y: parentCard.pos.y, w: CARD_W, h: CARD_H },
        { x: childCard.pos.x, y: childCard.pos.y, w: CARD_W, h: CARD_H },
        sitemapFrame
      );
    }

    // Draw group section labels (big text above each group's first row)
    const groupsByName = new Map<string, ManifestPage[]>();
    for (const page of pages) {
      if (!groupsByName.has(page.group)) groupsByName.set(page.group, []);
