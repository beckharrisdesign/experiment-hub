...
| 5 | ✅ 124 keywords, 15 months, on the table | [history note](pulls/2026-09-21-erank-keyword-history.md) | Katy ran the seven Bulk batches; the exports carry averages only, so the same batches were re-run and each chart read from its SVG. The 12-month means reproduce eRank's printed averages to the unit. The eRank band now carries `Peak Month` and `15-Month Trend`. 58 of 66 volume keywords are a single spike, 44 of them in Aug–Nov 2025; every treatment tag, captured phrase and `calm stitching` is flat at zero for fifteen months. |
