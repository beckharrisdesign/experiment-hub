...
- ☐ Score the verdict against **tag volume** as well: on the [Big Join read of 2026-09-21](ETSY_BIG_JOIN_READ_2026-09-21.md), the treatment arm's 13-tag sets sum to 0–3,594 eRank searches (Leaf Fan: zero tags scored) against 289–19,329 for the controls. A treatment win is also an eRank miss; a treatment loss is eRank being right. Say which.
...
- ☐ **Rewrite Geometric 4466080258's tags** — 7 of 13 censored `< 20`, 9 at KD 100, summed volume 150, yet it took two of the year's three $6 sales on phrases it is not tagged with (`hand embroidery pdf geometric`, `modern minimalist embroidery pattern`). Protected, so it waits. Rows and reasoning: [Big Join read, §D](ETSY_BIG_JOIN_READ_2026-09-21.md).
...
- ☐ **Five editable-now rewrites from the [Big Join read](ETSY_BIG_JOIN_READ_2026-09-21.md#d-listings-worth-rewriting)** — Photo Portrait Ornament 4522918821 (ornament-niche tags: `ultrasound ornament` 505 / 2,075 / KD 31), Pin 4555452273 (weakest tag set in the shop, do not clone the sibling's), Christmas Bow 4576478333 (`christmas embroidery designs` over `christmas wall art`), Christmas Classics set 4576496782 (`advent calendar` 8,347 / 33k / KD 30 if reframed as a stitch-along), Folk Leaf Circle 4466799018 (captured `mandala embroidery pattern` untagged). None is an experiment listing.
- ☐ **Consider the ultrasound ornament as a listing** — same engraved product, different image; the cheapest new variant the corpus surfaced.
- ☐ **When re-pulling resumes, run the eRank Tag Report first** — the 2026-09-15 report predates the holiday nine, so 8 of them show 1 tag of 13 scored. Tabled 2026-09-21 as manual; not urgent on the eRank side.
