...
| 8 | Ad spend | **Burst: $5/day × 3 days ≈ $15** on Google Search (revised from $1/day×14, same total — SSO playbook "run fast"; Katy approved burst 2026-07-27) | $1/day trickle; $0; higher budget | Faster signal in 3–4 days; plan in docs/AD_CAMPAIGN_GOOGLE.md. I prepare + cap; **Katy** creates/enables (money out) | No (build); enabling is Katy's | Low (pause) | APPROVED |
...
- ✅ **Supabase fully provisioned**: migration 007 (`elk_orders` RLS-on) + private buckets `elk-inputs`/`elk-outputs` (Experiment Hub 2.0, 2026-07-25). `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` + `ADMIN_SECRET` already in Vercel (redeploy to apply). **Supabase side done.**
